...
irisos.server_list = function(base_dir) {
  return fs.readdirSync(path.join(base_dir, irisos.DIRS['servers']));
}

irisos.server_list_up = function() {
  return Object.keys(irisos.server_pids_up());
}

irisos.server_pids_up = function() {
...
irisos.valid_server_name = function(server_name) {
  var regex_valid_server_name = /^(?!\.)[a-zA-Z0-9_\.]+$/;
  return regex_valid_server_name.test(server_name);
}

irisos.extract_server_name = function(base_dir, server_path) {
  var re = new RegExp('{0}/([a-zA-Z0-9_\.]+)'.format(path.join(base_dir, irisos.DIRS['servers'])));
  try {
...
irisos.dependencies = async.memoize(function(callback) {
  async.parallel({
...
irisos.mc = function(server_name, base_dir) {
  var self = this;
  ...
  self.env = {
    base_dir: base_dir,
    cwd: path.join(base_dir, irisos.DIRS['servers'], server_name),
    bwd: path.join(base_dir, irisos.DIRS['backup'], server_name),
    awd: path.join(base_dir, irisos.DIRS['archive'], server_name),
    pwd: path.join(base_dir, irisos.DIRS['profiles']),
    sp: path.join(base_dir, irisos.DIRS['servers'], server_name, 'server.properties'),
    sc: path.join(base_dir, irisos.DIRS['servers'], server_name, 'server.config'),
    cc: path.join(base_dir, irisos.DIRS['servers'], server_name, 'cron.config')
  }
  ...
  self.create = function(owner, callback) {
    async.series([
      ...
      async.apply(self.overlay_sp, irisos.SP_DEFAULTS),
  ...
  self.create_from_archive = function(owner, filepath, callback) {
    ...
    if (filepath.match(/\//))  //if it has a '/', its hopefully an absolute path
      ...
      var dest_filepath = path.join(self.env.base_dir, irisos.DIRS['import'], filepath);
        ...
        async.whilst(
          function() { 
            if (iterations > MAX_ITERATIONS_TO_QUIT)
              ...
              return (self.server_name in irisos.server_pids_up()) 
          },
          function(cc) { 
            iterations += 1;
          ...
          function(ignored_err) {
            if (self.server_name in irisos.server_pids_up())
  ...
  self.kill = function(callback) {
    var pids = irisos.server_pids_up();
    var test_interval_ms = 200;
    var MAX_ITERATIONS_TO_QUIT = 150;
    ...
    switch(property) {
      ...
      case 'memory':
        ...
        if (self.server_name in pids) {
          ...
          procfs.PROC = PROC_PATH; //procfs will default to /proc--this is determined more accurately by irisos.js!
