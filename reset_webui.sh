#!/bin/bash
# Reset irisos git repository to current master

LOG="/var/log/irisos.log"
...
ECHO_LOG_N "Updating irisos-node repository..."
git fetch >> $LOG
...
chmod +x irisos_console.js webui.js update_webui.sh reset_webui.sh >> $LOG
if [ $? -eq 0 ]; then ECHO_LOG "OK"; else ECHO_LOG "FAILED" && exit 1; fi
