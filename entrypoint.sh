...
if [ ! -z "$USE_HTTPS" ]; then
  # update irisos.conf from environment
  sed -i 's/use_https = .*/use_https = '${USE_HTTPS}'/g' /etc/irisos.conf
  ...
  sed -i 's/socket_port = .*/socket_port = '${Port}'/g' /etc/irisos.conf
  echo >&2 "Setting server port to: "$Port
fi

if [[ ! -f /etc/ssl/certs/irisos.crt ]] && [[ ! -z $( grep 'use_https = true' /etc/irisos.conf) ]]; then
...
if [ "$USE_HTTPS" ]; then
    if [ "$USE_HTTPS" == "true" ]; then
        ...
        sed -i 's/use_https = false/use_https = true/' /etc/irisos.conf

    else
        echo "Setting https to false"
        sed -i 's/use_https = true/use_https = false/' /etc/irisos.conf
    fi
else
