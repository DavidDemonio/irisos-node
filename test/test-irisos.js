...
function delete_everything(callback) {
  var server_list = new irisos.server_list(BASE_DIR);

  function delete_server(server_name, cb) {
    var instance = new irisos.mc(server_name, BASE_DIR);
...
test.setUp = function(callback) {
  async.parallel([
    async.apply(fs.ensureDir, path.join(BASE_DIR, irisos.DIRS['servers'])),
    async.apply(fs.ensureDir, path.join(BASE_DIR, irisos.DIRS['archive'])),
    async.apply(fs.ensureDir, path.join(BASE_DIR, irisos.DIRS['backup'])),
    async.apply(fs.ensureDir, path.join(BASE_DIR, irisos.DIRS['profiles'])),
  ], function() {
...
test.dependencies_met = function(test) {
  async.series([
    async.apply(irisos.dependencies)
...
test.server_list = function (test) {
  var servers = irisos.server_list(BASE_DIR);
  var instance = new irisos.mc('testing', BASE_DIR);

  instance.create(OWNER_CREDS, function(err, did_create) {
    servers = irisos.server_list(BASE_DIR);
    test.ifError(err);
...
test.server_list_up = function(test) {
  var servers = irisos.server_list_up();
...
test.create_server = function(test) {
  ...
  test.equal(irisos.server_list(BASE_DIR).length, 0);
  ...
  async.series([
    ...
    function(callback) {
      ...
      test.equal(irisos.server_list(BASE_DIR)[0], server_name);
      test.equal(irisos.server_list(BASE_DIR).length, 1);
...
test.mc_instance = function(test) {
  ...
  test.equal(instance.env.cwd, path.join(BASE_DIR, irisos.DIRS['servers'], server_name));
  test.equal(instance.env.bwd, path.join(BASE_DIR, irisos.DIRS['backup'], server_name));
  test.equal(instance.env.awd, path.join(BASE_DIR, irisos.DIRS['archive'], server_name));
  test.equal(instance.env.base_dir, BASE_DIR);
  test.equal(instance.server_name, server_name);
  test.equal(instance.env.sp, path.join(BASE_DIR, irisos.DIRS['servers'], server_name, 'server.properties'));
  test.done();
}

test.valid_server_name = function(test) {
  var regex_valid_server_name = /^(?!\.)[a-zA-Z0-9_\.]+$/;
  test.ok(irisos.valid_server_name('aaa'));
  test.ok(irisos.valid_server_name('server_1'));
  test.ok(irisos.valid_server_name('myserver'));
  test.ok(irisos.valid_server_name('1111'));
  test.ok(!irisos.valid_server_name('.aaa'));
  test.ok(!irisos.valid_server_name(''));
  test.ok(!irisos.valid_server_name('something!'));
  test.ok(!irisos.valid_server_name('#hashtag'));
  test.ok(!irisos.valid_server_name('my server'));
  test.ok(!irisos.valid_server_name('bukkit^ftb'));

  test.done();
}

test.extract_server_name = function(test) {
  test.equal(irisos.extract_server_name(BASE_DIR, '/home/runner/minecraft/servers/a'), 'a');
  test.equal(irisos.extract_server_name(BASE_DIR, '/home/runner/minecraft/servers/a/b'), 'a');
  test.equal(irisos.extract_server_name(BASE_DIR, '/home/runner/minecraft/servers/a/b/plugins'), 'a');
  test.throws(function(){irisos.extract_server_name(BASE_DIR, '/home/runner/minecraft/servers')}, 'no server name in path');
  test.throws(function(){irisos.extract_server_name(BASE_DIR, '/home/runner/minecraft')}, 'no server name in path');
  test.done();
}
...
test.sync_chown = function(test) {
  ...
  var server_name = 'testing';
  var server_path = path.join(BASE_DIR, irisos.DIRS['servers'], server_name);
  var instance = new irisos.mc(server_name, BASE_DIR);
...
test.create_server_from_awd = function(test) {
  var server_name = 'testing';
  var temporary_instance = new irisos.mc(server_name, BASE_DIR);
  var new_instance = new irisos.mc('testing_server_2', BASE_DIR);
