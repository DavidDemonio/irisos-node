...
var opt = getopt.create([
  ['c' , 'config_file=CONFIG_PATH'  , 'defaults to $PWD/custom.conf, then /etc/irisos.conf'],
  ...
  function(req, username, password, done) {
    ...
    .then(function (user) {
      if (user) {
        ...
        try {
          fs.appendFileSync('/var/log/irisos.auth.log', logstring);
        } catch (e) {
          console.log(e);
          console.log("Appending to local repo copy instead: ./irisos.auth.log");
          fs.appendFileSync('irisos.auth.log', logstring);
        }
        done(null, user);
      }
    })
...
irisos.dependencies(function(err, binaries) {
  if (err) {
    console.error('irisos is missing dependencies:', err);
  ...
  var config_locs = ['custom.conf',
                     '/etc/irisos.conf',
                     '/usr/local/etc/irisos.conf']

  var irisos_config = {};
  if (typeof config_file !== 'undefined') {
    console.info('using command-line provided configuration identified as', config_file);
    irisos_config = read_ini(config_file);
  } else {
    for (var loc in config_locs) {
      try {
        fs.statSync(config_locs[loc]);
        console.info('first irisos configuration identified as', config_locs[loc]);
        irisos_config = read_ini(config_locs[loc])
        break;
      } catch (e) {}
    }
  }
  ...
  if ('base_directory' in irisos_config) {
    try {
      if (irisos_config['base_directory'].length < 2)
        throw new error('Invalid base_directory length.');

      base_directory = irisos_config['base_directory'];
    ...
    console.error('base_directory not specified--missing irisos.conf?');
    console.error('alternatively, you can make custom.conf in the repository root directory');
    console.error('Aborting startup.');
    process.exit(4); 
  }

  var be = new server.backend(base_directory, io, irisos_config);
  ...
  if ('use_https' in irisos_config)
    USE_HTTPS = irisos_config['use_https'];

  if ('socket_host' in irisos_config)
    SOCKET_HOST = irisos_config['socket_host'];

  if ('socket_port' in irisos_config)
    SOCKET_PORT = irisos_config['socket_port'];
  else
    if (USE_HTTPS)
      SOCKET_PORT = 8443;
    else
  ...
  if (USE_HTTPS) {
    keyfile = irisos_config['ssl_private_key'] || '/etc/ssl/certs/irisos.key';
    certfile = irisos_config['ssl_certificate'] || '/etc/ssl/certs/irisos.crt';
    async.parallel({
      ...
      if (err) {
        ...
        if ('ssl_cert_chain' in irisos_config) {
          try {
            var cert_chain_data = fs.readFileSync(irisos_config['ssl_cert_chain']);
        ...
        var https_server = https.createServer(ssl, app).listen(SOCKET_PORT, SOCKET_HOST, function() {
          ...
          console.log('irisos webui listening on HTTPS://' + SOCKET_HOST + ':' + SOCKET_PORT);
        });
      }
    })
  } else {
    console.warn('irisos.conf set to host insecurely: starting HTTP server.');
    http.listen(SOCKET_PORT, SOCKET_HOST, function(){
      console.log('irisos webui listening on HTTP://' + SOCKET_HOST + ':' + SOCKET_PORT);
    });
  }
