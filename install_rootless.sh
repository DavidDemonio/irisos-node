...
git clone https://github.com/DavidDemonio/irisos-node
cd irisos-node
git config core.filemode false
chmod +x generate-sslcert.sh irisos_console.js webui.js

# PLACE AND CONFIGURE IRISOS.CONF
mkdir -p ~/.local/etc/ssl/certs/
cp irisos.conf ~/.local/etc/irisos.conf
sed -i "s./var/games/minecraft.$HOME/minecraft.g" ~/.local/etc/irisos.conf
sed -i "s./etc/ssl/certs.$HOME/\.local/etc/ssl/certs.g" ~/.local/etc/irisos.conf

# GENERATE SSL CERTS
mkdir -p ~/.local/etc/ssl/certs
SSL_PATH=~/.local/etc/ssl/certs
CERTFILE=$SSL_PATH/irisos.pem CRTFILE=$SSL_PATH/irisos.crt KEYFILE=$SSL_PATH/irisos.key ./generate-sslcert.sh
...
mkdir -p ~/.bashrc.d
echo "export PATH=\$PATH:$HOME/nsolid/nsolid-fermium/bin:$JDK_PATH" > ~/.bashrc.d/irisos
source ~/.bashrc.d/irisos
