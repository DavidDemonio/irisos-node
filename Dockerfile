FROM ubuntu:focal
LABEL MAINTAINER='David Martinez <davidmartinez2021xd@gmail.com>'
...
RUN mkdir /usr/games/minecraft \
  ...
  && git clone --depth=1 https://github.com/DavidDemonio/irisos-node.git . \
  && cp mineos.conf /etc/irisos.conf \
  && chmod +x webui.js irisos_console.js service.js
...
RUN cp /usr/games/minecraft/init/supervisor_conf /etc/supervisor/conf.d/irisos.conf
