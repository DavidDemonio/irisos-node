...
server.backend = function(base_dir, socket_emitter, user_config) {
  ...
  fs.ensureDirSync(path.join(base_dir, irisos.DIRS['servers']));
  fs.ensureDirSync(path.join(base_dir, irisos.DIRS['backup']));
  fs.ensureDirSync(path.join(base_dir, irisos.DIRS['archive']));
  fs.ensureDirSync(path.join(base_dir, irisos.DIRS['import']));
  fs.ensureDirSync(path.join(base_dir, irisos.DIRS['profiles']));

  fs.chmod(path.join(base_dir, irisos.DIRS['import']), 0777);
  ...
  (function() {
    var server_path = path.join(base_dir, irisos.DIRS['servers']);
  ...
  (function() {
    ...
    var importable_archives = path.join(base_dir, irisos.DIRS['import']);
  ...
  self.send_profile_list = function(send_existing) {
    if (send_existing && self.profiles.length) //if requesting to just send what you already have AND they are already present
      ...
      var profile_dir = path.join(base_dir, irisos.DIRS['profiles']);
  ...
  self.send_spigot_list = function() {
    var profiles_dir = path.join(base_dir, irisos.DIRS['profiles']);
    var spigot_profiles = {};
  ...
  self.front_end.on('connection', function(socket) {
    ...
    function webui_dispatcher (args) {
      ...
      switch (args.command) {
        case 'create':
          var instance = new irisos.mc(args.server_name, base_dir);
            ...
            if (self.profiles[idx].id == args.profile.id) {
              ...
              async.series([
                ...
                function(cb) {
                  ...
                  var request = require('request');
                  progress(request({url: args.profile.url, headers: {'User-Agent': 'irisos-node'}}), { throttle: 250, delay: 100 })
        ...
        case 'build_jar':
          ...
          try {
            var profile_path = path.join(base_dir, irisos.DIRS['profiles']);
        ...
        case 'delete_build':
          if (args.type == 'spigot')
            var spigot_path = path.join(base_dir, irisos.DIRS['profiles'], 'spigot_' + args.version);
          else {
        ...
        case 'copy_to_server':
          ...
          var dest_path = path.join(base_dir, irisos.DIRS['servers'], args.server_name) + '/';
        ...
        case 'create_from_archive':
          var instance = new irisos.mc(args.new_server_name, base_dir);

          if (args.awd_dir)
            var filepath = path.join(instance.env.base_dir, irisos.DIRS['archive'], args.awd_dir, args.filename);
          else
            var filepath = path.join(instance.env.base_dir, irisos.DIRS['import'], args.filename);
...
function server_container(server_name, user_config, socket_io) {
  ...
  var self = this;
  var instance = new irisos.mc(server_name, user_config.base_directory),
